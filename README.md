...
这是一个仿「大众点评」的后端学习实战项目，主要围绕 **Redis** 的各种应用场景展开，涵盖短信登录、商户缓存、优惠券秒杀、社交关注、附近商户、用户签到、UV 统计等核心业务功能。项目以 Spring Boot 为基础框架，配合 MySQL 持久化存储与 Redis 高性能缓存，辅以 Nginx 做前端静态资源服务和反向代理，是一个前后端分离的单体应用。
...
在浏览器中打开 `http://localhost:8080`，即可进入前端页面。
